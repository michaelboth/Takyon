...
if [ "$socket" == "yes" ]; then
    ...
    if [ "$endpoint" == "A" ]; then
        ...
        sleep 1
        echo ""
        ./throughput_mp $endpoint "SocketUdpSend -multicast -localIP=$local_ip -groupIP=233.23.33.56 -port=23458" -n=10000 -b=1024 -e
        if [ $? -ne 0 ]; then echo "Failed to run example"; exit 1; fi
    else
        echo ""
        ...
        if [ $? -ne 0 ]; then echo "Failed to run example"; exit 1; fi
        echo ""
        ./throughput_mp $endpoint "SocketUdpRecv -multicast -localIP=$local_ip -groupIP=233.23.33.56 -port=23458 -reuse" -n=10000 -b=1024 -e
        if [ $? -ne 0 ]; then echo "Failed to run example"; exit 1; fi
    fi
fi
...
if [ "$rdma" == "yes" ]; then
    ...
    if [ "$endpoint" == "A" ]; then
        ...
        if [ $? -ne 0 ]; then echo "Failed to run example"; exit 1; fi
        echo ""
        ./throughput_mp $endpoint "RdmaUDUnicastSend -client -remoteIP=$remote_ip -port=23457 -rdmaDevice=mlx5_0 -rdmaPort=1 -gidIndex=0" -n=1000000 -b=4096 -s=100
        if [ $? -ne 0 ]; then echo "Failed to run example"; exit 1; fi
        echo ""
        ./throughput_mp $endpoint "RdmaUDUnicastSend -client -remoteIP=$remote_ip -port=23458 -rdmaDevice=mlx5_0 -rdmaPort=1 -gidIndex=0" -n=1000000 -b=4096 -s=100 -e
        if [ $? -ne 0 ]; then echo "Failed to run example"; exit 1; fi
    else
        echo ""
        ...
        if [ $? -ne 0 ]; then echo "Failed to run example"; exit 1; fi
        echo ""
        ./throughput_mp $endpoint "RdmaUDUnicastRecv -server -localIP=$local_ip -port=23457 -reuse -rdmaDevice=mlx5_0 -rdmaPort=1 -gidIndex=0" -n=1000000 -b=4136 -r=1000
        if [ $? -ne 0 ]; then echo "Failed to run example"; exit 1; fi
        echo ""
        ./throughput_mp $endpoint "RdmaUDUnicastRecv -server -localIP=$local_ip -port=23458 -reuse -rdmaDevice=mlx5_0 -rdmaPort=1 -gidIndex=0" -n=1000000 -b=4136 -r=1000 -e
        if [ $? -ne 0 ]; then echo "Failed to run example"; exit 1; fi
    fi
