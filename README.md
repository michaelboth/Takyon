...
rdma_vs_socket | A simple latency and throughput test to compare the performance difference between RDMA and sockets. This test is specifically designed for RDMA RC (reliable connected), RDMA UC (unreliable connected), and TCP sockets. It optionally uses GPU memory for the transport buffers, and if validation is turned on, then CUDA kernels are used to do the validation.
