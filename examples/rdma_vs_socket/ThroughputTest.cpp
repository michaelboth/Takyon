...
void ThroughputTest::runThroughputTest(bool _is_sender, const Common::AppParams &_app_params) {
  ...
  for (uint32_t i=0; i<send_request_count; i++) {
    ...
    send_sub_buffers[i].bytes = _is_sender ? message_bytes : ACK_BYTES;
    send_sub_buffers[i].offset = _is_sender ? i * message_bytes : i * ACK_BYTES;
    // Request
  ...
  do {
    ...
    for (uint64_t curr_message_bytes=min_message_bytes; curr_message_bytes<=max_message_bytes; curr_message_bytes*=2) {
      ...
      if (cycle_message_sizes) {
        printf("\n");
        print_start_time = -Common::ELAPSED_SECONDS_TO_PRINT; // Make sure new pass prints
        accumulated_throughput_Gbps = -1.0;
      }
    } // End of curr_message_bytes for loop
